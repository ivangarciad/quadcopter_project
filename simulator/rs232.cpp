...
int RS232_OpenComport(int comport_number, int baudrate)
{
  ...
  printf("Serial port: %s\n Baudrate: %d\n", comports[comport_number], baudr);

  Cport[comport_number] = open(comports[comport_number], O_RDWR | O_NOCTTY | O_NONBLOCK);
  ...
  new_port_settings.c_lflag = ~(ICANON | ECHO | ECHOE);
  new_port_settings.c_cc[VMIN] = 24;     /* block untill n bytes are received */
...
void RS232_fflush(int comport_number)
{
   fflush((FILE*)Cport[comport_number]);
}
